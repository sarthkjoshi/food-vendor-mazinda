...
const VendorOrdersPage = () => {
  ...
  const orderFetcher = async () => {
    try {
      ...
      const undeliveredOrders = ordersWithUsers.filter(
        (order) => order.status !== "Delivered"
      );

      return { orders: undeliveredOrders };
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-5 md:w-1/2">
      <h1 className="text-3xl font-semibold mb-8 text-center">Vendor Orders</h1>
      ...
      <div className="w-full flex justify-center">
        <a
          className="bg-black text-white px-3 py-2 rounded-md mt-5"
          href="orders/delivered-orders"
        >
